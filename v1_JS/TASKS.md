...
- [x] Helper `parseItem()` documentado em [web/app.js](web/app.js)
...
**Go geral:** sintaxe, structs, error handling, packages. Não precisas de concorrência avançada — só o essencial.
Recurso: [A Tour of Go](https://go.dev/tour) (faz até "Concurrency" inclusive mas focado).

**Especificamente para o bridge + world loader:**
- [ ] **`net/http`** — `http.HandleFunc`, `ServeMux`, `ListenAndServe`, servir ficheiros estáticos com `http.FileServer`
- [ ] **WebSocket lib (Go)** — escolher entre [`nhooyr.io/websocket`](https://pkg.go.dev/nhooyr.io/websocket) (moderna) ou [`gorilla/websocket`](https://pkg.go.dev/github.com/gorilla/websocket) (clássica). Ler o exemplo de echo server da que escolheres.
- [ ] **`net.Dial`** — abrir conexão TCP do bridge para o servidor TAP
- [ ] **`io.Copy` ou pumps em goroutines** — relay bidirecional entre WS e TCP
- [ ] **`encoding/json`** — `Unmarshal` para ler `world.json`
- [ ] **`os.ReadFile`** — ler ficheiros de disco

**JavaScript (browser):**
- [ ] **WebSocket API** — `new WebSocket(url)`, `onmessage`, `onopen`, `send()`
- [ ] **DOM manipulation** — `document.getElementById`, `querySelector`, `classList`, `addEventListener`, criar elementos com `createElement`
- [ ] **JSON** — `JSON.parse()` para as respostas que o servidor envia

**CSS:**
- [ ] **Flexbox** ou **Grid** — para os painéis (room, chat, inventory, quests) lado a lado
- [ ] **Tabs** — switching entre Global/Room/Group no chat (display:none + classes)
...
- [ ] [go.mod](go.mod) — `go get nhooyr.io/websocket` (ou gorilla) e apagar o comentário
- [ ] [cmd/gui/main.go](cmd/gui/main.go) — `main()`, `serveHome()`, `handleWS()`, `relayWStoTCP()`, `relayTCPtoWS()`
- [ ] [web/style.css](web/style.css) — styling base (layout, painéis, tabs de chat)
- [ ] [web/index.html](web/index.html) — preencher os TODO comments (status-bar, exits, items, NPCs, chat tabs, inventory, quests)

### Bloco 2 — Esqueleto do `app.js` (não precisa do servidor a correr)
- [ ] [web/app.js](web/app.js) — `connect()`, `sendCommand()`, `buildMoveCommand()`, `buildChatCommand()`
- [ ] [web/app.js](web/app.js) — `handleMessage()` a fazer routing entre `handleResponse` e `handleEvent`
- [ ] [web/app.js](web/app.js) — `renderRoom()`, `renderExits()`, `renderInventory()`

> 🚦 **Checkpoint de integração** com o Rui. Vertical slice end-to-end.

### Bloco 3 — Resto do `app.js` (já com o servidor a correr)
- [ ] [web/app.js](web/app.js) — `handleLookResponse()` (usar `parseItem()`)
- [ ] [web/app.js](web/app.js) — `handleInventoryResponse()` (usar `parseItem()`)
- [ ] [web/app.js](web/app.js) — `handleStatusResponse()`
- [ ] [web/app.js](web/app.js) — `handleRoomPresenceEnter()`, `handleRoomPresenceLeave()`
- [ ] [web/app.js](web/app.js) — `appendChatMessage()` (Global/Room/Group)
- [ ] [web/app.js](web/app.js) — `updatePlayerCount()`
- [ ] Botões de ATTACK / TALK / QUEST / QUESTS no UI
...
- [ ] Mudar a **cor** das mensagens de chat por scope em [web/style.css](web/style.css)
...
- [ ] Adicionar um **novo botão** de ação no GUI (HTML + handler no JS)
